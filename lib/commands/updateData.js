	...
	.then ((array) => {
		...
		api.getStationsData(function(err, devices) {
			devices.forEach(function(device) {
				...
				.then((row) => {
					gladys.deviceState.create( { 'value' : device.battery_percent , 'devicetype' : row[0].id })
				})

				gladys.utils.sql( 
					queries.getDeviceTypeId, 
					[ device._id, 'Température' ]
				)
				.then((row) => {
					gladys.deviceState.create( { 'value' : device.dashboard_data.Temperature , 'devicetype' : row[0].id })
				})
				...
				.then((row) => {
					gladys.deviceState.create( { 'value' : device.dashboard_data.Humidity , 'devicetype' : row[0].id })
				})
				...
				.then((row) => {
					gladys.deviceState.create( { 'value' : device.dashboard_data.Pressure , 'devicetype' : row[0].id })
				})
				...
				.then((row) => {
					gladys.deviceState.create( { 'value' : device.dashboard_data.CO2 , 'devicetype' : row[0].id })
				})
				...
				.then((row) => {
					gladys.deviceState.create( { 'value' : device.dashboard_data.Noise , 'devicetype' : row[0].id })
				})
